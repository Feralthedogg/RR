...
Module RRPipelineFnCfgOpenSearchSummaryProtocolSubset.

Definition open_search_summary_trace := list adaptive_summary.

Record src_fn_cfg_open_search_summary_protocol_program : Type := {
  src_open_summary_rounds : list src_fn_cfg_open_search_meta_iter_program;
  src_open_summary_stable : adaptive_summary;
}.

Record mir_fn_cfg_open_search_summary_protocol_program : Type := {
  mir_open_summary_rounds : list mir_fn_cfg_open_search_meta_iter_program;
  mir_open_summary_stable : adaptive_summary;
}.

Record r_fn_cfg_open_search_summary_protocol_program : Type := {
  r_open_summary_rounds : list r_fn_cfg_open_search_meta_iter_program;
  r_open_summary_stable : adaptive_summary;
}.

Fixpoint last_open_search_summary_trace (traces : open_search_summary_trace) : adaptive_summary :=
  match traces with
  | [] => []
  | [summary] => summary
  | _ :: rest => last_open_search_summary_trace rest
  end.

Definition r_open_search_summary_protocol_witness
    (p : r_fn_cfg_open_search_summary_protocol_program) : Prop :=
  last_open_search_summary_trace
    (List.map eval_r_fn_cfg_open_search_meta_iter_program (r_open_summary_rounds p)) =
    r_open_summary_stable p.

Definition lower_fn_cfg_open_search_summary_protocol_program
    (p : src_fn_cfg_open_search_summary_protocol_program)
    : mir_fn_cfg_open_search_summary_protocol_program :=
  {| mir_open_summary_rounds := map lower_fn_cfg_open_search_meta_iter_program (src_open_summary_rounds p);
     mir_open_summary_stable := src_open_summary_stable p |}.

Definition emit_r_fn_cfg_open_search_summary_protocol_program
    (p : mir_fn_cfg_open_search_summary_protocol_program)
    : r_fn_cfg_open_search_summary_protocol_program :=
  {| r_open_summary_rounds := map emit_r_fn_cfg_open_search_meta_iter_program (mir_open_summary_rounds p);
     r_open_summary_stable := mir_open_summary_stable p |}.

Definition eval_r_fn_cfg_open_search_summary_protocol_program
    (p : r_fn_cfg_open_search_summary_protocol_program) : open_search_summary_trace :=
  map eval_r_fn_cfg_open_search_meta_iter_program (r_open_summary_rounds p).

Definition stable_fn_cfg_open_search_summary_protocol_program :
    src_fn_cfg_open_search_summary_protocol_program :=
  {| src_open_summary_rounds :=
       [stable_fn_cfg_open_search_meta_iter_program;
        stable_fn_cfg_open_search_meta_iter_program];
     src_open_summary_stable := stable_closed_loop_summary |}.

Lemma stable_fn_cfg_open_search_summary_protocol_program_meta_preserved :
  List.length
    (mir_open_summary_rounds
      (lower_fn_cfg_open_search_summary_protocol_program
        stable_fn_cfg_open_search_summary_protocol_program)) = 2%nat /\
  mir_open_summary_stable
    (lower_fn_cfg_open_search_summary_protocol_program
      stable_fn_cfg_open_search_summary_protocol_program) =
    stable_closed_loop_summary.
Proof.
  split; reflexivity.
Qed.

Lemma stable_fn_cfg_open_search_summary_protocol_program_eval_preserved :
  eval_r_fn_cfg_open_search_summary_protocol_program
    (emit_r_fn_cfg_open_search_summary_protocol_program
      (lower_fn_cfg_open_search_summary_protocol_program
        stable_fn_cfg_open_search_summary_protocol_program)) =
...
Lemma stable_fn_cfg_open_search_summary_protocol_program_preserved :
  r_open_search_summary_protocol_witness
    (emit_r_fn_cfg_open_search_summary_protocol_program
      (lower_fn_cfg_open_search_summary_protocol_program
        stable_fn_cfg_open_search_summary_protocol_program)).
Proof.
...
End RRPipelineFnCfgOpenSearchSummaryProtocolSubset.
