...
Module RRPipelineFnCfgOpenSearchFrontierSubset.

Record src_fn_cfg_open_search_frontier_program : Type := {
  src_open_frontier_halt_prog : src_fn_cfg_open_search_halt_discover_program;
  src_open_frontier_completed : list adaptive_summary;
  src_open_frontier_frontier : list adaptive_summary;
}.

Record mir_fn_cfg_open_search_frontier_program : Type := {
  mir_open_frontier_halt_prog : mir_fn_cfg_open_search_halt_discover_program;
  mir_open_frontier_completed : list adaptive_summary;
  mir_open_frontier_frontier : list adaptive_summary;
}.

Record r_fn_cfg_open_search_frontier_program : Type := {
  r_open_frontier_halt_prog : r_fn_cfg_open_search_halt_discover_program;
  r_open_frontier_completed : list adaptive_summary;
  r_open_frontier_frontier : list adaptive_summary;
}.

Definition r_open_search_frontier_witness
    (p : r_fn_cfg_open_search_frontier_program) : Prop :=
  r_open_search_halt_discover_witness (r_open_frontier_halt_prog p) /\
  r_open_halt_search_space (r_open_frontier_halt_prog p) =
    List.app (r_open_frontier_completed p) (r_open_frontier_frontier p) /\
  In (r_open_halt_selected (r_open_frontier_halt_prog p)) (r_open_frontier_frontier p).

Definition lower_fn_cfg_open_search_frontier_program
    (p : src_fn_cfg_open_search_frontier_program)
    : mir_fn_cfg_open_search_frontier_program :=
  {| mir_open_frontier_halt_prog :=
       lower_fn_cfg_open_search_halt_discover_program (src_open_frontier_halt_prog p);
     mir_open_frontier_completed := src_open_frontier_completed p;
     mir_open_frontier_frontier := src_open_frontier_frontier p |}.

Definition emit_r_fn_cfg_open_search_frontier_program
    (p : mir_fn_cfg_open_search_frontier_program)
    : r_fn_cfg_open_search_frontier_program :=
  {| r_open_frontier_halt_prog :=
       emit_r_fn_cfg_open_search_halt_discover_program (mir_open_frontier_halt_prog p);
     r_open_frontier_completed := mir_open_frontier_completed p;
     r_open_frontier_frontier := mir_open_frontier_frontier p |}.

Definition eval_r_fn_cfg_open_search_frontier_program
    (p : r_fn_cfg_open_search_frontier_program) : adaptive_summary :=
  eval_r_fn_cfg_open_search_halt_discover_program (r_open_frontier_halt_prog p).

Definition stable_fn_cfg_open_search_frontier_program :
    src_fn_cfg_open_search_frontier_program :=
  {| src_open_frontier_halt_prog := stable_fn_cfg_open_search_halt_discover_program;
     src_open_frontier_completed := [[]];
     src_open_frontier_frontier := [stable_closed_loop_summary] |}.

Lemma stable_fn_cfg_open_search_frontier_program_meta_preserved :
  List.length
    (mir_open_summary_rounds
      (mir_open_conv_summary_prog
        (mir_open_halt_protocol_prog
          (mir_open_frontier_halt_prog
            (lower_fn_cfg_open_search_frontier_program
              stable_fn_cfg_open_search_frontier_program))))) = 2%nat /\
  mir_open_frontier_completed
    (lower_fn_cfg_open_search_frontier_program stable_fn_cfg_open_search_frontier_program) =
    [[]] /\
  mir_open_frontier_frontier
    (lower_fn_cfg_open_search_frontier_program stable_fn_cfg_open_search_frontier_program) =
...
Lemma stable_fn_cfg_open_search_frontier_program_eval_preserved :
  eval_r_fn_cfg_open_search_frontier_program
    (emit_r_fn_cfg_open_search_frontier_program
      (lower_fn_cfg_open_search_frontier_program stable_fn_cfg_open_search_frontier_program)) =
    stable_closed_loop_summary.
Proof.
  reflexivity.
...
Lemma stable_fn_cfg_open_search_frontier_program_preserved :
  r_open_search_frontier_witness
    (emit_r_fn_cfg_open_search_frontier_program
      (lower_fn_cfg_open_search_frontier_program stable_fn_cfg_open_search_frontier_program)).
Proof.
  split.
  - exact stable_fn_cfg_open_search_halt_discover_program_preserved.
  - split.
    + reflexivity.
    + simpl. tauto.
Qed.

End RRPipelineFnCfgOpenSearchFrontierSubset.
