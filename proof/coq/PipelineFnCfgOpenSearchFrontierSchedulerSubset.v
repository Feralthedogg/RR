...
Module RRPipelineFnCfgOpenSearchFrontierSchedulerSubset.

Record src_fn_cfg_open_search_frontier_scheduler_program : Type := {
  src_open_sched_dyn_prog : src_fn_cfg_open_search_dynamic_frontier_program;
  src_open_sched_future_rounds : list (list adaptive_summary);
  src_open_sched_scheduled_rounds : list (list adaptive_summary);
}.

Record mir_fn_cfg_open_search_frontier_scheduler_program : Type := {
  mir_open_sched_dyn_prog : mir_fn_cfg_open_search_dynamic_frontier_program;
  mir_open_sched_future_rounds : list (list adaptive_summary);
  mir_open_sched_scheduled_rounds : list (list adaptive_summary);
}.

Record r_fn_cfg_open_search_frontier_scheduler_program : Type := {
  r_open_sched_dyn_prog : r_fn_cfg_open_search_dynamic_frontier_program;
  r_open_sched_future_rounds : list (list adaptive_summary);
  r_open_sched_scheduled_rounds : list (list adaptive_summary);
}.

Definition r_open_search_frontier_scheduler_witness
    (p : r_fn_cfg_open_search_frontier_scheduler_program) : Prop :=
  r_open_search_dynamic_frontier_witness (r_open_sched_dyn_prog p) /\
  r_open_sched_scheduled_rounds p =
    (r_dyn_frontier_next (r_open_sched_dyn_prog p)) :: r_open_sched_future_rounds p /\
  In (r_open_halt_selected (r_open_frontier_halt_prog (r_dyn_frontier_prog (r_open_sched_dyn_prog p))))
    (r_dyn_frontier_next (r_open_sched_dyn_prog p)).

Definition lower_fn_cfg_open_search_frontier_scheduler_program
    (p : src_fn_cfg_open_search_frontier_scheduler_program)
    : mir_fn_cfg_open_search_frontier_scheduler_program :=
  {| mir_open_sched_dyn_prog :=
       lower_fn_cfg_open_search_dynamic_frontier_program (src_open_sched_dyn_prog p);
     mir_open_sched_future_rounds := src_open_sched_future_rounds p;
     mir_open_sched_scheduled_rounds := src_open_sched_scheduled_rounds p |}.

Definition emit_r_fn_cfg_open_search_frontier_scheduler_program
    (p : mir_fn_cfg_open_search_frontier_scheduler_program)
    : r_fn_cfg_open_search_frontier_scheduler_program :=
  {| r_open_sched_dyn_prog :=
       emit_r_fn_cfg_open_search_dynamic_frontier_program (mir_open_sched_dyn_prog p);
     r_open_sched_future_rounds := mir_open_sched_future_rounds p;
     r_open_sched_scheduled_rounds := mir_open_sched_scheduled_rounds p |}.

Definition eval_r_fn_cfg_open_search_frontier_scheduler_program
    (p : r_fn_cfg_open_search_frontier_scheduler_program) : adaptive_summary :=
  eval_r_fn_cfg_open_search_dynamic_frontier_program (r_open_sched_dyn_prog p).

Definition stable_fn_cfg_open_search_frontier_scheduler_program :
    src_fn_cfg_open_search_frontier_scheduler_program :=
  {| src_open_sched_dyn_prog := stable_fn_cfg_open_search_dynamic_frontier_program;
     src_open_sched_future_rounds := [[stable_closed_loop_summary]];
     src_open_sched_scheduled_rounds := [[stable_closed_loop_summary; []]; [stable_closed_loop_summary]] |}.

Lemma stable_fn_cfg_open_search_frontier_scheduler_program_meta_preserved :
  List.length
        ...
        (mir_open_halt_protocol_prog
          (mir_open_frontier_halt_prog
            (mir_dyn_frontier_prog
              (mir_open_sched_dyn_prog
                (lower_fn_cfg_open_search_frontier_scheduler_program
                  stable_fn_cfg_open_search_frontier_scheduler_program))))))) = 2%nat /\
  mir_open_sched_future_rounds
    (lower_fn_cfg_open_search_frontier_scheduler_program
      stable_fn_cfg_open_search_frontier_scheduler_program) =
    [[stable_closed_loop_summary]] /\
  mir_open_sched_scheduled_rounds
    (lower_fn_cfg_open_search_frontier_scheduler_program
      stable_fn_cfg_open_search_frontier_scheduler_program) =
...
Lemma stable_fn_cfg_open_search_frontier_scheduler_program_eval_preserved :
  eval_r_fn_cfg_open_search_frontier_scheduler_program
    (emit_r_fn_cfg_open_search_frontier_scheduler_program
      (lower_fn_cfg_open_search_frontier_scheduler_program
        stable_fn_cfg_open_search_frontier_scheduler_program)) =
    stable_closed_loop_summary.
Proof.
...
Lemma stable_fn_cfg_open_search_frontier_scheduler_program_preserved :
  r_open_search_frontier_scheduler_witness
    (emit_r_fn_cfg_open_search_frontier_scheduler_program
      (lower_fn_cfg_open_search_frontier_scheduler_program
        stable_fn_cfg_open_search_frontier_scheduler_program)).
Proof.
  split.
  - exact stable_fn_cfg_open_search_dynamic_frontier_program_preserved.
  - split.
    + reflexivity.
    + simpl. tauto.
Qed.

End RRPipelineFnCfgOpenSearchFrontierSchedulerSubset.
