...
Module RRPipelineFnCfgOpenSearchMetaIterSubset.

Definition last_open_search_summary : adaptive_open_search_trace -> adaptive_summary :=
  fix go traces :=
    match traces with
    | [] => []
    | [trace] => trace
...
Record src_fn_cfg_open_search_meta_iter_program : Type := {
  src_meta_closed_prog : src_fn_cfg_open_search_closed_loop_program;
  src_meta_summary : adaptive_summary;
}.

Record mir_fn_cfg_open_search_meta_iter_program : Type := {
  mir_meta_closed_prog : mir_fn_cfg_open_search_closed_loop_program;
  mir_meta_summary : adaptive_summary;
}.

Record r_fn_cfg_open_search_meta_iter_program : Type := {
  r_meta_closed_prog : r_fn_cfg_open_search_closed_loop_program;
  r_meta_summary : adaptive_summary;
}.

Definition mir_open_search_closed_loop_round_count
    (p : mir_fn_cfg_open_search_closed_loop_program) : nat :=
  match p with
  | Build_mir_fn_cfg_open_search_closed_loop_program rounds => List.length rounds
  end.

Definition lower_fn_cfg_open_search_meta_iter_program
    (p : src_fn_cfg_open_search_meta_iter_program)
    : mir_fn_cfg_open_search_meta_iter_program :=
  {| mir_meta_closed_prog := lower_fn_cfg_open_search_closed_loop_program (src_meta_closed_prog p);
     mir_meta_summary := src_meta_summary p |}.

Definition emit_r_fn_cfg_open_search_meta_iter_program
    (p : mir_fn_cfg_open_search_meta_iter_program)
    : r_fn_cfg_open_search_meta_iter_program :=
  {| r_meta_closed_prog := emit_r_fn_cfg_open_search_closed_loop_program (mir_meta_closed_prog p);
     r_meta_summary := mir_meta_summary p |}.

Definition eval_r_fn_cfg_open_search_meta_iter_program
    (p : r_fn_cfg_open_search_meta_iter_program) : adaptive_summary :=
  last_open_search_summary (eval_r_fn_cfg_open_search_closed_loop_program (r_meta_closed_prog p)).

Definition stable_fn_cfg_open_search_meta_iter_program :
    src_fn_cfg_open_search_meta_iter_program :=
  {| src_meta_closed_prog := stable_fn_cfg_open_search_closed_loop_program;
     src_meta_summary := stable_closed_loop_summary |}.

Lemma stable_fn_cfg_open_search_meta_iter_program_meta_preserved :
  mir_open_search_closed_loop_round_count
    (mir_meta_closed_prog
      (lower_fn_cfg_open_search_meta_iter_program stable_fn_cfg_open_search_meta_iter_program)) = 2%nat /\
  mir_meta_summary
    (lower_fn_cfg_open_search_meta_iter_program stable_fn_cfg_open_search_meta_iter_program) =
    stable_closed_loop_summary.
Proof.
  split; reflexivity.
Qed.

Lemma stable_fn_cfg_open_search_meta_iter_program_eval_preserved :
  eval_r_fn_cfg_open_search_meta_iter_program
    (emit_r_fn_cfg_open_search_meta_iter_program
      (lower_fn_cfg_open_search_meta_iter_program stable_fn_cfg_open_search_meta_iter_program)) =
    stable_closed_loop_summary.
Proof.
  reflexivity.
...
End RRPipelineFnCfgOpenSearchMetaIterSubset.
