...
Module RRPipelineFnCfgOpenSearchHaltDiscoverSubset.

Record src_fn_cfg_open_search_halt_discover_program : Type := {
  src_open_halt_protocol_prog : src_fn_cfg_open_search_convergence_protocol_program;
  src_open_halt_search_space : list adaptive_summary;
  src_open_halt_selected : adaptive_summary;
}.

Record mir_fn_cfg_open_search_halt_discover_program : Type := {
  mir_open_halt_protocol_prog : mir_fn_cfg_open_search_convergence_protocol_program;
  mir_open_halt_search_space : list adaptive_summary;
  mir_open_halt_selected : adaptive_summary;
}.

Record r_fn_cfg_open_search_halt_discover_program : Type := {
  r_open_halt_protocol_prog : r_fn_cfg_open_search_convergence_protocol_program;
  r_open_halt_search_space : list adaptive_summary;
  r_open_halt_selected : adaptive_summary;
}.

Definition r_open_search_halt_discover_witness
    (p : r_fn_cfg_open_search_halt_discover_program) : Prop :=
  r_open_search_convergence_protocol_witness (r_open_halt_protocol_prog p) /\
  In (r_open_halt_selected p) (r_open_halt_search_space p) /\
  r_open_halt_selected p = r_open_conv_halt_summary (r_open_halt_protocol_prog p).

Definition lower_fn_cfg_open_search_halt_discover_program
    (p : src_fn_cfg_open_search_halt_discover_program)
    : mir_fn_cfg_open_search_halt_discover_program :=
  {| mir_open_halt_protocol_prog :=
       lower_fn_cfg_open_search_convergence_protocol_program (src_open_halt_protocol_prog p);
     mir_open_halt_search_space := src_open_halt_search_space p;
     mir_open_halt_selected := src_open_halt_selected p |}.

Definition emit_r_fn_cfg_open_search_halt_discover_program
    (p : mir_fn_cfg_open_search_halt_discover_program)
    : r_fn_cfg_open_search_halt_discover_program :=
  {| r_open_halt_protocol_prog :=
       emit_r_fn_cfg_open_search_convergence_protocol_program (mir_open_halt_protocol_prog p);
     r_open_halt_search_space := mir_open_halt_search_space p;
     r_open_halt_selected := mir_open_halt_selected p |}.

Definition eval_r_fn_cfg_open_search_halt_discover_program
    (p : r_fn_cfg_open_search_halt_discover_program) : adaptive_summary :=
  r_open_conv_halt_summary (r_open_halt_protocol_prog p).

Definition stable_fn_cfg_open_search_halt_discover_program :
    src_fn_cfg_open_search_halt_discover_program :=
  {| src_open_halt_protocol_prog := stable_fn_cfg_open_search_convergence_protocol_program;
     src_open_halt_search_space := [[]; stable_closed_loop_summary];
     src_open_halt_selected := stable_closed_loop_summary |}.

Lemma stable_fn_cfg_open_search_halt_discover_program_meta_preserved :
  List.length
    (mir_open_summary_rounds
      (mir_open_conv_summary_prog
        (mir_open_halt_protocol_prog
          (lower_fn_cfg_open_search_halt_discover_program
            stable_fn_cfg_open_search_halt_discover_program)))) = 2%nat /\
  mir_open_halt_search_space
    (lower_fn_cfg_open_search_halt_discover_program
      stable_fn_cfg_open_search_halt_discover_program) =
    [[]; stable_closed_loop_summary] /\
  mir_open_halt_selected
    (lower_fn_cfg_open_search_halt_discover_program
      stable_fn_cfg_open_search_halt_discover_program) =
    stable_closed_loop_summary.
Proof.
...
Lemma stable_fn_cfg_open_search_halt_discover_program_eval_preserved :
  eval_r_fn_cfg_open_search_halt_discover_program
    (emit_r_fn_cfg_open_search_halt_discover_program
      (lower_fn_cfg_open_search_halt_discover_program
        stable_fn_cfg_open_search_halt_discover_program)) =
    stable_closed_loop_summary.
Proof.
...
Lemma stable_fn_cfg_open_search_halt_discover_program_preserved :
  r_open_search_halt_discover_witness
    (emit_r_fn_cfg_open_search_halt_discover_program
      (lower_fn_cfg_open_search_halt_discover_program
        stable_fn_cfg_open_search_halt_discover_program)).
Proof.
  split.
  - exact stable_fn_cfg_open_search_convergence_protocol_program_preserved.
  - split.
    + simpl. tauto.
    + reflexivity.
Qed.

End RRPipelineFnCfgOpenSearchHaltDiscoverSubset.
