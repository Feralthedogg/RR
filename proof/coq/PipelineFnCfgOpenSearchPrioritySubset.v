...
Module RRPipelineFnCfgOpenSearchPrioritySubset.

Definition open_search_priority_round : Type := (nat * list adaptive_summary)%type.

Record src_fn_cfg_open_search_priority_program : Type := {
  src_prio_sched_prog : src_fn_cfg_open_search_scheduler_program;
  src_prio_tail : list open_search_priority_round;
  src_prio_rounds : list open_search_priority_round;
}.

Record mir_fn_cfg_open_search_priority_program : Type := {
  mir_prio_sched_prog : mir_fn_cfg_open_search_scheduler_program;
  mir_prio_tail : list open_search_priority_round;
  mir_prio_rounds : list open_search_priority_round;
}.

Record r_fn_cfg_open_search_priority_program : Type := {
  r_prio_sched_prog : r_fn_cfg_open_search_scheduler_program;
  r_prio_tail : list open_search_priority_round;
  r_prio_rounds : list open_search_priority_round;
}.

Definition r_open_search_priority_witness (p : r_fn_cfg_open_search_priority_program) : Prop :=
  r_open_search_scheduler_witness (r_prio_sched_prog p) /\
  r_prio_rounds p =
    (5%nat, r_dyn_next_frontier (r_sched_dyn_prog (r_prio_sched_prog p))) :: r_prio_tail p /\
  In (r_halt_selected (r_open_halt_prog (r_dyn_open_prog (r_sched_dyn_prog (r_prio_sched_prog p)))))
    (r_dyn_next_frontier (r_sched_dyn_prog (r_prio_sched_prog p))).

Definition lower_fn_cfg_open_search_priority_program
    (p : src_fn_cfg_open_search_priority_program) : mir_fn_cfg_open_search_priority_program :=
  {| mir_prio_sched_prog := lower_fn_cfg_open_search_scheduler_program (src_prio_sched_prog p);
     mir_prio_tail := src_prio_tail p;
     mir_prio_rounds := src_prio_rounds p |}.

Definition emit_r_fn_cfg_open_search_priority_program
    (p : mir_fn_cfg_open_search_priority_program) : r_fn_cfg_open_search_priority_program :=
  {| r_prio_sched_prog := emit_r_fn_cfg_open_search_scheduler_program (mir_prio_sched_prog p);
     r_prio_tail := mir_prio_tail p;
     r_prio_rounds := mir_prio_rounds p |}.

Definition eval_r_fn_cfg_open_search_priority_program
    (p : r_fn_cfg_open_search_priority_program) : adaptive_summary :=
  eval_r_fn_cfg_open_search_scheduler_program (r_prio_sched_prog p).

Definition stable_fn_cfg_open_search_priority_program :
    src_fn_cfg_open_search_priority_program :=
  {| src_prio_sched_prog := stable_fn_cfg_open_search_scheduler_program;
     src_prio_tail := [(3%nat, [stable_closed_loop_summary])];
     src_prio_rounds := [(5%nat, [stable_closed_loop_summary; []]); (3%nat, [stable_closed_loop_summary])] |}.

Lemma stable_fn_cfg_open_search_priority_program_meta_preserved :
  List.length (mir_summary_rounds
    (mir_conv_summary_prog (mir_halt_protocol_prog (mir_open_halt_prog (mir_dyn_open_prog
      (mir_sched_dyn_prog (mir_prio_sched_prog
        (lower_fn_cfg_open_search_priority_program stable_fn_cfg_open_search_priority_program)))))))) = 2%nat /\
  mir_prio_tail (lower_fn_cfg_open_search_priority_program stable_fn_cfg_open_search_priority_program) =
    [(3%nat, [stable_closed_loop_summary])] /\
  mir_prio_rounds (lower_fn_cfg_open_search_priority_program stable_fn_cfg_open_search_priority_program) =
    [(5%nat, [stable_closed_loop_summary; []]); (3%nat, [stable_closed_loop_summary])].
Proof.
  repeat split; reflexivity.
Qed.

Lemma stable_fn_cfg_open_search_priority_program_eval_preserved :
  eval_r_fn_cfg_open_search_priority_program
    (emit_r_fn_cfg_open_search_priority_program
      (lower_fn_cfg_open_search_priority_program stable_fn_cfg_open_search_priority_program)) =
    stable_closed_loop_summary.
Proof.
  reflexivity.
Qed.

Lemma stable_fn_cfg_open_search_priority_program_preserved :
  r_open_search_priority_witness
    (emit_r_fn_cfg_open_search_priority_program
      (lower_fn_cfg_open_search_priority_program stable_fn_cfg_open_search_priority_program)).
Proof.
  split.
  - exact stable_fn_cfg_open_search_scheduler_program_preserved.
  - split.
    + reflexivity.
    + simpl. tauto.
Qed.

End RRPipelineFnCfgOpenSearchPrioritySubset.
