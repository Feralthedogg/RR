...
#[test]
fn conditional_map_partial_range_vectorizes_with_slice_assign() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let out_dir = root
        .join("target")
        .join("tests")
        .join("vectorization_conditional_slice");
...
print(cond_slice(8, 4))
"#;

    let rr_path = out_dir.join("cond_slice.rr");
    let o0 = out_dir.join("cond_slice_o0.R");
    let o1 = out_dir.join("cond_slice_o1.R");
    let o2 = out_dir.join("cond_slice_o2.R");
    ...
    assert!(
        o1_code.contains("rr_ifelse_strict("),
        "expected strict ifelse helper in vectorized output"
    );
    assert!(
