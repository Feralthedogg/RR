...
fn compile_with_tile3d(name: &str, rr_src: &str) -> (String, String) {
...
#[test]
fn poly_tiles_nested_3d_map_when_policy_enabled() {
    let (stats, emitted) = compile_with_tile3d(
        "poly_tile_3d_map",
        r#"
import r * as base from "base"

fn poly_tile_3d_map(a, b, out) {
  let i = 1
  while (i <= 3) {
    let j = 1
    while (j <= 3) {
...
print(poly_tile_3d_map(a, b, out))
"#,
    );
    assert!(
    ...
    assert!(
        ...
        "expected tiled 3d map to rebuild generic loops without tile helper, got:\n{}",
        emitted
    );
}

#[test]
fn poly_tiles_nested_3d_fused_maps_when_policy_enabled() {
    let (stats, emitted) = compile_with_tile3d(
        "poly_tile_3d_multi_map",
        r#"
import r * as base from "base"

fn poly_tile_3d_multi_map(a, b, y, z) {
  let i = 1
  while (i <= 3) {
    let j = 1
    while (j <= 3) {
...
print(poly_tile_3d_multi_map(a, b, y, z))
"#,
    );
    assert!(
        stats.contains("\"poly_schedule_applied\": 1")
    ...
    assert!(
        !emitted.contains("rr_tile_array3_binop_cube_assign("),
        "expected fused tiled 3d maps to rebuild generic loops without tile helpers, got:\n{}",
        emitted
    );
}

#[test]
fn poly_tiles_nested_3d_reduction_when_policy_enabled() {
    let (stats, emitted) = compile_with_tile3d(
        "poly_tile_3d_reduce",
        r#"
import r * as base from "base"

fn poly_tile_3d_reduce(a) {
  let acc = 0
  let i = 1
  while (i <= 3) {
    let j = 1
...
"#,
    ...
    assert!(
        ...
        "expected tiled 3d reduction to rebuild generic loops without tile helper, got:\n{}",
        emitted
    );
}

#[test]
fn poly_tiles_nested_3d_fused_reductions_when_policy_enabled() {
    let (stats, emitted) = compile_with_tile3d(
        "poly_tile_3d_multi_reduce",
        r#"
import r * as base from "base"

fn poly_tile_3d_multi_reduce(a, b) {
  let sum_acc = 0
  let prod_acc = 1
  let i = 1
  while (i <= 3) {
...
let a = base.array(seq_len(27), base.c(3, 3, 3))
let b = base.array(seq_len(27), base.c(3, 3, 3))
print(poly_tile_3d_multi_reduce(a, b))
"#,
    );
    assert!(
        stats.contains("\"poly_schedule_applied\": 1")
    ...
    assert!(
        !emitted.contains("rr_tile_array3_reduce_cube("),
        "expected fused tiled 3d reductions to rebuild generic loops without tile helpers, got:\n{}",
        emitted
    );
}

#[test]
fn poly_tiles_nested_3d_fused_minmax_reductions_when_policy_enabled() {
    let (stats, emitted) = compile_with_tile3d(
        "poly_tile_3d_multi_minmax_reduce",
        r#"
import r * as base from "base"

fn poly_tile_3d_multi_minmax_reduce(a) {
  let min_acc = 100
  let max_acc = 0
  let i = 1
  while (i <= 3) {
...
let a = base.array(seq_len(27), base.c(3, 3, 3))
print(poly_tile_3d_multi_minmax_reduce(a))
"#,
    );
    assert!(
        stats.contains("\"poly_schedule_applied\": 1")
    ...
    assert!(
        !emitted.contains("rr_tile_array3_reduce_cube("),
        "expected fused tiled 3d minmax reductions to rebuild generic loops without tile helpers, got:\n{}",
        emitted
    );
    assert!(
