...
#[test]
fn partial_range_call_map_vectorizes_with_slice_assign() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let out_dir = root
        .join("target")
        .join("tests")
        .join("vectorization_callmap_slice");
...
"#;
    ...
    let rr_path = out_dir.join("call_slice.rr");
    let o0 = out_dir.join("call_slice_o0.R");
    let o1 = out_dir.join("call_slice_o1.R");
    let o2 = out_dir.join("call_slice_o2.R");
    ...
    assert!(
        o1_code.contains("abs("),
        "expected abs call in vectorized output"
    );
    assert!(
