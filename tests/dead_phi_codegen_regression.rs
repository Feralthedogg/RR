...
fn top_level_compound_assign_with_dead_phi_compiles() {
    ...
    let (ok_o1, stdout_o1, stderr_o1) =
        common::run_compile_case("dead_phi_codegen_regression", source, "case.rr", "-O1", &[]);
    assert!(
        ok_o1,
        "O1 compile failed\nstdout:\n{stdout_o1}\nstderr:\n{stderr_o1}"
    );
    ...
    let (ok_o2, stdout_o2, stderr_o2) =
        common::run_compile_case("dead_phi_codegen_regression", source, "case.rr", "-O2", &[]);
    assert!(
        ok_o2,
        "O2 compile failed\nstdout:\n{stdout_o2}\nstderr:\n{stderr_o2}"
    );
