...
#[test]
fn multi_expr_map_restores_shadow_state() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let out_dir = root
        .join("target")
        .join("tests")
        .join("vectorization_multi_shadow");
...
print(multi_shadow(5))
"#;

    let rr_path = out_dir.join("multi_shadow.rr");
    let o0 = out_dir.join("multi_shadow_o0.R");
    let o1 = out_dir.join("multi_shadow_o1.R");
    let o2 = out_dir.join("multi_shadow_o2.R");
