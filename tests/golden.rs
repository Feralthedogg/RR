...
use common::{
    RunResult, compile_rr_env_with_args, normalize, rscript_available, rscript_path, run_rscript,
};
use std::fs;
...
fn golden_r_semantics() {
    ...
    for rr_path in rr_files {
        ...
        assert!(
            ref_r.exists(),
            "missing golden R baseline for {}",
            rr_path.display()
        );
        let ref_run = run_rscript(&rscript, &ref_r);
