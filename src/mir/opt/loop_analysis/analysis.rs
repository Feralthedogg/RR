...
impl<'a> LoopAnalyzer<'a> {
    ...
    pub(crate) fn analyze_natural_loop(&self, header: BlockId, latch: BlockId) -> Option<LoopInfo> {
    ...
    pub(crate) fn compute_limit_adjust(&self, header: BlockId, iv: &InductionVar) -> i64 {
    ...
    pub(crate) fn find_induction_variable(
        &self,
        header: BlockId,
    ...
    pub(crate) fn find_counter_iv_from_condition(
        &self,
        header: BlockId,
        body: &FxHashSet<BlockId>,
    ) -> (Option<InductionVar>, Option<ValueId>) {
    ...
    pub(crate) fn normalize_floor_like_value(&self, mut vid: ValueId) -> ValueId {
        loop {
            let ValueKind::Call {
                callee,
                args,
    ...
    pub(crate) fn call_is_single_positional(
        &self,
        args: &[ValueId],
        names: &[Option<String>],
    ) -> bool {
    ...
    pub(crate) fn call_is_floor_like(
        &self,
        vid: ValueId,
        callee: &str,
        args: &[ValueId],
    ...
    pub(crate) fn find_seed_assignment_outside_loop(
        &self,
        var: &str,
        body: &FxHashSet<BlockId>,
    ) -> Option<ValueId> {
    ...
    pub(crate) fn find_var_step_in_loop(
        &self,
        var: &str,
        iv_val: ValueId,
        body: &FxHashSet<BlockId>,
    ...
    pub(crate) fn analyze_step(&self, val_id: ValueId, phi_id: ValueId) -> Option<(i64, BinOp)> {
    ...
    pub(crate) fn analyze_step_from_var(
        &self,
        var: &str,
        phi_id: ValueId,
        seen_vals: &mut FxHashSet<ValueId>,
    ...
    pub(crate) fn value_depends_on_phi(&self, root: ValueId, phi_id: ValueId) -> bool {
        pub(crate) fn rec(
    ...
    pub(crate) fn load_var_is_phi_equivalent(
        &self,
        var: &str,
        phi_id: ValueId,
        seen_vals: &mut [bool],
    ...
    pub(crate) fn const_integral_value(&self, vid: ValueId) -> Option<i64> {
        pub(crate) fn rec(
            analyzer: &LoopAnalyzer<'_>,
            vid: ValueId,
            seen_vals: &mut FxHashSet<ValueId>,
    ...
    pub(crate) fn resolve_len_base(&self, vid: ValueId) -> Option<ValueId> {
        let mut seen_vals = FxHashSet::default();
        let mut seen_vars = FxHashSet::default();
        self.resolve_len_base_rec(vid, &mut seen_vals, &mut seen_vars)
    }

    pub(crate) fn resolve_condition_compare(
        &self,
        vid: ValueId,
    ...
    pub(crate) fn resolve_condition_compare_rec(
        &self,
        vid: ValueId,
        seen_vals: &mut FxHashSet<ValueId>,
        seen_vars: &mut FxHashSet<String>,
    ...
    pub(crate) fn resolve_unique_assignment(
        &self,
        var: &str,
        seen_vals: &mut FxHashSet<ValueId>,
        seen_vars: &mut FxHashSet<String>,
    ...
    pub(crate) fn resolve_len_base_rec(
        &self,
        vid: ValueId,
        seen_vals: &mut FxHashSet<ValueId>,
        seen_vars: &mut FxHashSet<String>,
    ...
    pub(crate) fn resolve_len_base_from_var(
        &self,
        var: &str,
        seen_vals: &mut FxHashSet<ValueId>,
        seen_vars: &mut FxHashSet<String>,
    ...
    // Helpers
    pub(crate) fn get_cfg_edges(&self) -> Vec<(BlockId, Vec<BlockId>)> {
        self.fn_ir
            .blocks
            .iter()
            .map(|b| {
    ...
    pub(crate) fn get_block_successors(&self, bid: BlockId) -> Vec<BlockId> {
    ...
    pub(crate) fn compute_dominators(&self) -> FxHashMap<BlockId, FxHashSet<BlockId>> {
...
pub(crate) fn flip_compare(op: BinOp) -> BinOp {
    match op {
        BinOp::Le => BinOp::Ge,
        BinOp::Lt => BinOp::Gt,
        BinOp::Ge => BinOp::Le,
