...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct BranchOnlyCondReductionEntry {
    pub(crate) plan: ReduceCondEntry,
    pub(crate) merge_phi: ValueId,
}

pub(crate) fn certify_simple_reduction(
    fn_ir: &FnIR,
    lp: &LoopInfo,
...
pub(crate) fn certify_reduction_candidate<F>(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    acc_phi: ValueId,
    other: ValueId,
...
pub(crate) fn certify_simple_cond_reduction(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    user_call_whitelist: &FxHashSet<String>,
) -> Result<CertifiedPlan, ProofFallbackReason> {
...
pub(crate) fn certify_cond_reduction_candidate(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    acc_phi: ValueId,
    other: ValueId,
...
pub(crate) fn certify_branch_only_multi_cond_reduction_candidate(
    fn_ir: &FnIR,
    lp: &LoopInfo,
...
pub(crate) fn classify_cond_reduction_store_branches(
    fn_ir: &FnIR,
    then_bb: usize,
    else_bb: usize,
    iv_phi: ValueId,
...
pub(crate) fn branch_only_cond_reduction_has_non_acc_loop_state_assignments(
    fn_ir: &FnIR,
    lp: &LoopInfo,
...
pub(crate) fn reduction_update_for_acc(
    fn_ir: &FnIR,
    acc_phi: ValueId,
    next_val: ValueId,
...
pub(crate) fn classify_branch_only_cond_reduction_entries(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    then_bb: usize,
    else_bb: usize,
...
pub(crate) fn extract_cond_reduce_arm(
    fn_ir: &FnIR,
    acc_phi: ValueId,
    root: ValueId,
...
pub(crate) fn direct_acc_identity_value(fn_ir: &FnIR, root: ValueId, acc_phi: ValueId) -> bool {
    let root = preserve_phi_value(fn_ir, root);
...
pub(crate) fn resolve_cond_reduce_branch_values(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    acc_phi: ValueId,
...
pub(crate) fn cond_reduce_identity_value(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    acc_phi: ValueId,
    kind: planning::ReduceKind,
...
pub(crate) fn cond_reduction_has_blocking_extra_state_phi(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    allowed_acc_phis: &[ValueId],
    dest_base: Option<ValueId>,
...
pub(crate) fn reduction_reads_current_dest_element(
    fn_ir: &FnIR,
    lp: &LoopInfo,
    root: ValueId,
    dest_base: ValueId,
...
pub(crate) fn operand_matches_acc_phi(fn_ir: &FnIR, operand: ValueId, acc_phi: ValueId) -> bool {
