...
use self::flags::{CommonCompileFlagState, apply_common_compile_flags};
use self::incremental::parse_incremental_phases;
pub(crate) use self::types::{CommandMode, CommonOpts};

pub(crate) fn parse_command_opts(
    args: &[String],
    mode: CommandMode,
    ui: &CliLog,
            ...
            match apply_common_compile_flags(args, &mut i, &mut compile_flag_state, ui) {
                ...
                Ok(false) => {
                    if mode.allow_keep_r() && arg == "--keep-r" {
                        ...
                        if i + 1 >= args.len() {
                            ui.error("Missing value after --profile-use");
                            return Err(1);
                        }
                        i += 1;
                        opts.profile_use = Some(args[i].clone());
    ...
    if matches!(
        mode,
        CommandMode::Run | CommandMode::Build | CommandMode::Watch
    ) && !opts.compile_mode_explicit
        && matches!(opts.opt_level, OptLevel::O2 | OptLevel::O3 | OptLevel::Oz)
    {
