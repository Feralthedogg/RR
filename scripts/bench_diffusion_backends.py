...
DEFAULT_OUT_DIR = ROOT / "target" / "diffusion_backend_bench"
...
WORKLOADS = {
    "heat": {
        "src": ROOT / "example" / "benchmarks" / "heat_diffusion_bench.rr",
        "metrics": ["heat_bench_center", "heat_bench_energy"],
        "title": "Heat Diffusion",
    },
    "reaction": {
        "src": ROOT / "example" / "benchmarks" / "reaction_diffusion_bench.rr",
        "metrics": ["rd_bench_center", "rd_bench_mass"],
...
def write_svg_chart(path: pathlib.Path, rows: list[dict[str, object]]) -> None:
    ...
    lines = [
        ...
        '  <title id="title">Diffusion Optimizer Tier Comparison</title>',
        '  <desc id="desc">Mean wall-clock runtime in milliseconds for heat and reaction diffusion workloads across RR optimization tiers.</desc>',
        ...
        f'  <text x="24" y="56" font-family="Helvetica, Arial, sans-serif" font-size="12" fill="#4b5b67">Heat and reaction diffusion workloads, Apple M4, RR O0/O1/O2 cold and warm timing</text>',
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark RR optimizer tiers on diffusion workloads.")
    ...
    json_path = out_dir / "diffusion_backend_bench.json"
    csv_path = out_dir / "diffusion_backend_bench.csv"
    svg_path = out_dir / "diffusion_backend_bench.svg"
